...
\begin{document}
    ...
    \lstinputlisting[language=sql, title=task\_1.cql]{../scripts/task_1.cql}
    \lstinputlisting[language=sql, title=task\_2.cql]{../scripts/task_2.cql}
    \lstinputlisting[language=sql, title=task\_3.cql]{../scripts/task_3.cql}
    \lstinputlisting[language=sql, title=task\_3.py]{../task_3.py}
...
docker exec -it cassandra-node1 cqlsh cassandra-node1 -f /scripts/task_3.cql
        ...
        \begin{enumerate}
            \item Вказавши у параметрах запиту Consistency Level One (це буде означати, що запис відбувається синхронно тільки на одну ноду),
                ...
                \begin{lstlisting}[language=python, style=log]
